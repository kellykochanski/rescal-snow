...
#ifdef PARALLEL
        if ((pt->celltype != BORD) && (pt->celltype != TUNNEL)){
#else
        if (pt->celltype != BORD){
#endif
...
void write_csp(char dump_type, char *filename)
{
  ...
  if (dump_type==DUMP_CSP){
    ...
    if (start) LogPrintf("header size: %d\n", hd_size);
    start = 0;
  }
  ...
  if (dump_type==DUMP_CSP) fwrite(header, 1, hd_size, fp);
  ...
  for(j=0; j< D; j++){
    ...
    for(i=0; i< HL; i++, pt++){
      ...
      if (pt->celltype != BORD){
#endif
        //*aux++ = (char) pt->celltype;
        *aux++ = *pt;
      }
    }
    if (aux>buf) fwrite(buf, sizeof(Cell), csp_H*csp_L, fp);
  }
  fclose(fp);
