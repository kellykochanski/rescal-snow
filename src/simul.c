...
#ifdef CELL_COLOR
extern float grdvc_max_col; //seuil max pour le controle de l'erosion des grains colores en fonction du gradient de vitesse
extern float grdvc_min_col; //seuil min pour le controle de l'erosion des grains colores en fonction du gradient de vitesse
#endif
#endif
#endif
...
//double temps=0.0;                  // temps reel simule
#ifdef TIME_SCALE
...
void params_simul()
{
  ...
  //parameter_hidden("Ava_anisotropy", "Ava_anisotropy parameter is obsolete - Please use Ava_upwind parameter instead !", &ava_upwind, PARAM_INT, "AVALANCHES");
#ifdef CELL_COLOR
  parameter("Ava_angle_col", "angle of repose for white grains (degrees) - optional", &ava_angle_col, PARAM_FLOAT, "COLOR");
  parameter("Ava_angle_unstable_col", "unstable angle of repose for white grains (degrees) - optional", &ava_angle_unstable_col, PARAM_FLOAT, "COLOR");
#endif
#endif //AVALANCHES

#ifdef CELL_COLOR
  parameter("Color_coef", "control coefficient of colored grains - optional", &color_coef, PARAM_FLOAT, "COLOR");
#endif

#ifdef SURRECTIONS
  param_family("SURRECTION", "Surrections parameters");
  parameter("Sur_mode", "surrection mode (UNIFORM|CONE|TECTO)", &sur_mode_str, PARAM_STRING, "SURRECTION");
  parameter("Sur_delay", "surrection delay", &sur_delay, PARAM_FLOAT, "SURRECTION");
#endif
...
#ifdef CGV
//  parameter("Grdvc", "critical shear stress for relationship between erosion rate and shear stress - optional", &grdvc, PARAM_FLOAT);
...
#ifdef CELL_COLOR
  parameter("Tau_min_col", "min shear stress value for white grains in the linear relationship between erosion rate and shear stress - optional", &grdvc_min_col, PARAM_FLOAT, "COLOR");
  parameter("Tau_max_col", "max shear stress value for white grains in the linear relationship between erosion rate and shear stress - optional", &grdvc_max_col, PARAM_FLOAT, "COLOR");
  parameter_hidden("Grdvc_min_col", "Grdvc_min_col parameter is obsolete - Please use Tau_min_col parameter instead !", &grdvc_min_col, PARAM_FLOAT, "COLOR");
  parameter_hidden("Grdvc_max_col", "Grdvc_max_col parameter is obsolete - Please use Tau_max_col parameter instead !", &grdvc_max_col, PARAM_FLOAT, "COLOR");
#endif
...
#ifdef SURRECTIONS
  if (sur_mode_str){
    if (!strcmp(sur_mode_str, "UNIFORM")){
      sur_mode = SUR_MODE_UNIFORM;
    }
    else if (!strcmp(sur_mode_str, "CONE")){
      sur_mode = SUR_MODE_CONE;
    }
    else if (!strcmp(sur_mode_str, "TECTO")){
      sur_mode = SUR_MODE_TECTO;
    }
    else{
      ErrPrintf("Incorrect value for Sur_mode : %s\n", sur_mode_str);
      exit(-2);
    }
  }
#endif
...
inline void compute_prob_dist()
{
  ...
  //LogPrintf("calcul_prob: start\n");

  // doublet transitions
  ptr = t_trans;
  for(i=0; i<nb_trans_db; i++, ptr++){
    //regulation of a transition
    if (ptr->regul) ptr->regul(&ptr->intensite);
    PI_ij[i] = (double) Ndb[ptr->depart]*ptr->intensite;
#ifdef CGV
    //if (t_trans[i].check) PI_ij[i] *= cgv_coef;
#endif
    //if (PI_ij[i]) LogPrintf("tr = %d   rate = %f   time = %f\n", i, ptr->intensite, csp_time); //debug
  }

  // cell transitions
  ptr_cel = t_trans_cel;
  for(; i<nb_trans; i++,ptr_cel++){
    if (ptr_cel->regul) ptr_cel->regul(&ptr_cel->intensite); //regulation de la transition
...
/*#ifdef CGV
    //if ((i<nb_trans_db) && (t_trans[i].check != NULL)) PII_time  += PI_ij[i]*cgv_coef; else PII_time  += PI_ij[i];
    if ((i<nb_trans_db) && (t_trans[i].time_mode == TIME_CORR)) PII_time  += PI_ij[i]*cgv_coef; else PII_time  += PI_ij[i];
#endif*/
    cum[i] = PI_ij[i] + ccc;
    ccc    = cum[i];
    PII_time += PI_ij_time;
...
void simul_time()
{
  ...
  if (PII_time){
    aleat = drand48();
    if (!aleat) {ErrPrintf("ERROR: simul_time - aleat = 0 !\n"); exit(-1); }
...
inline int simul_trans()
{
  ...
  /*#ifdef TRACE
      ...
      //linked transitions
      if (t_trans[tr].lien) simul_lien_trans(tr, ix, dir);
  #endif

      flag_evol_time = (t_trans[tr].time_mode != TIME_NO_EVOL);
      if (flag_evol_time) cpt_blk = 0; /// reset counter when last transition makes time evolve
    }
...
void simul_time_scale(float wind_value)
{
  ...
  if (start){
    ...
    for(i=0; i<nb_rows; i++){
      ...
      //LogPrintf("%d\t%f\t%f\t%f\n", i, tau1_values[i], erosion_rates[i], qsat_ratios[i]); //debug
    }
  ...
  //float tau1; //threshold of the shear stress
  ...
  if (wind_value >= 0){
    ...
    if (i<nb_rows){
      //grdvc_min = (tau1_values[i-1]+tau1_values[i])/2; //here we use PDF.data to get tau1 according to the value Qsat_ratio[i]
    ...
    if (grdvc_min == 0) grdvc_min = 1.0; //value 0 is not allowed
    grdvc_max = grdvc_min + 1000.0;
    LogPrintf("grdvc_min = %f\n", grdvc_min);
    //LogPrintf("time_scale = %f\n", time_scale);
  }
  else{
...
void simul_correl(int tr1, int ix, int ix2, int dir)
{
  ...
  if (n > 0){ //test tableau non-vide ?
    ...
    if (i < n){
      ...
      cpt_lien[ln[i]]++;
#endif
      if (t_trans[tr2].lien) simul_lien_trans(tr2, ind[i], dir);
    }
  }
}
...
void simul_sur()
{
  ...
  if (sur_delay){
    if (!time_threshold){
      time_threshold = sur_delay*ceil(csp_time/sur_delay);
      LogPrintf("delai de surrection = %f\n", sur_delay);
    }
...
void simul_rot()
{
  ...
  if (start){
    ...
    if (!seq_flag){
      ...
      if (time_threshold == csp_time) time_threshold += rot_delay;
    }
    ...
    if (seq_flag) LogPrintf("period od the sequence: %d\n", lg_seq);
#ifdef LGCA
    if (use_lgca){
  ...
  if (csp_time >= time_threshold){
    ...
    if (seq_flag){
      ...
      //nseq = (short)lg_seq*drand48(); //random direction in sequence
      nseq = cpt_rot % lg_seq;
...
//#ifdef CGV
    ...
    grdvc_min = seq_gvc[nseq];
    grdvc_max = grdvc_min + 1000.0;
    LogPrintf("variation d'intensite du vent : grdvc_min = %f   grdvc_max = %f   csp_time = %f\n", grdvc_min, grdvc_max, csp_time);
#endif

    //conditions_bords();
    //float delai_rot_alea = delai_rot*1.0*seq_del[nseq];
    //float delai_rot_alea = rot_delay*drand48();
    //float delai_rot_alea = delai_rot*0.5*(1.0+drand48());
    //time_threshold += delai_rot_alea;
    if (seq_flag){
      time_threshold += rot_delay*seq_del[nseq];
    }
    else
      time_threshold += rot_delay;
    //time_threshold = 10000000L;

    //LogPrintf("cpt_rot = %d   delai_rot = %f   total_seq_rot = %d   coef_injection = %f\n", cpt_rot, rot_delay, total_seq_rot, coef_injection);
    LogPrintf("cpt_rot = %d\n", cpt_rot);
...
void simul_centering()
{
  ...
  if (start){
    ...
    //if (time_threshold == csp_time) time_threshold += centering_delay;
...
void simul_ava_sync()
{
  ...
  if (start){
    ...
    inter_iter_ava = 1;
#endif
    //inter_iter_ava *= (float)(LNS/100.0);
    //ava_delay_sync = 1.0 / lambda_A;
    ava_delay_sync = ava_delay;

    time_threshold = ava_delay_sync*ceil(csp_time/ava_delay_sync);//*drand48();
    iter_0 = iter;
    md_iter_0 = md_iter;
...
//#if !defined(DUMP_SIGMA) && !defined(DUMP_AUTOCORREL)
#if 0
  if ((!csp_time) || ((csp_time >= time_threshold) && ((iter-iter_0 >= inter_iter_ava) || (md_iter-md_iter_0 > 0)))){
#else
...
//  static float ava_delay_long = 3000.0;
  ...
  if (ava_mode == AVA_PROPAG){
    if (start){
      ...
      if (ava_angle_unstable) {ava_angle = ava_angle_unstable; mode_stable = 0;}
#ifdef CELL_COLOR
      if (ava_angle_unstable_col) {ava_angle_col = ava_angle_unstable_col; mode_stable = 0;}
#endif
      if (lambda_A_unstable) {lambda_A = lambda_A_unstable; mode_stable = 0;}
      start = 0;
    }
    return;
  }
  ...
  if (start){
    time_threshold = ava_duration*ceil(csp_time/ava_duration);//*drand48();
    ava_angle_stable = ava_angle;
    lambda_A_stable = lambda_A;
    //ava_angle_unstable = atan(1.5*tan(ava_angle*PI/180.0))*180/PI; //pente * 1.5
    ava_angle_stable_col = ava_angle_col;
    //ava_angle_unstable_col = atan(1.2*tan(ava_angle_col*PI/180.0))*180/PI;
...
#ifdef CELL_COLOR
    LogPrintf("ava_angle_col = %f (stable)| %f (unstable)\n", ava_angle_stable_col, ava_angle_unstable_col);
#endif
    //LogPrintf("nb min iterations avant avalanches = %lu\n", inter_iter_ava);
    start = 0;
  }
  ...
  if ((!csp_time) || (csp_time >= time_threshold)){
    ...
    //LogPrintf("ava_angle = %f\n",ava_angle);
    //LogPrintf("lambda_A = %f\n",lambda_A);
#ifdef CELL_COLOR
    if (ava_angle_col == ava_angle_stable_col){
      ava_angle_col = ava_angle_unstable_col;
    }
    else{
      ava_angle_col = ava_angle_stable_col;
    }
    //LogPrintf("ava_angle_col = %f\n",ava_angle_col);
#endif //CELL_COLOR
  }
}
#endif //AVALANCHES
...
#ifdef CGV
//extern float lambda_E; //erosion rate
extern float grdvc_max; //seuil max pour le controle de l'erosion en fonction du gradient de vitesse
extern float grdvc_min; //seuil min pour le controle de l'erosion en fonction du gradient de vitesse

void simul_wind_variability()
{
  static float delai_vent_max = 200.0; //1000.0; //10.0;
  static float time_threshold = 0.0;
  ...
  if (start){
    ...
    //lambda_E_debut = lambda_E;
    //pente_erosion_grdv = lambda_E/(grdvc_max-grdvc_min);
    //LogPrintf("parametres d'intensite du vent au debut : lambda_E = %f   grdvc_min = %f   grdvc_max = %f\n", lambda_E, grdvc_min, grdvc_max);
    //LogPrintf("pente_erosion_grdv = %f\n", pente_erosion_grdv);
    LogPrintf("parametres d'intensite du vent au debut : grdvc_min = %f   grdvc_max = %f\n", grdvc_min, grdvc_max);
    LogPrintf("delai max de variation du vent = %f\n", delai_vent_max);
    start = 0;
  }
  if (csp_time >= time_threshold){
    //lambda_E = 2.0*lambda_E_debut*drand48();
    //lambda_E = lambda_E_debut/(1.1-drand48());
    //grdvc_min = grdvc_max - lambda_E/pente_erosion_grdv;
    grdvc_min = drand48()*500.0;
    grdvc_max = grdvc_min + 1000.0;
    float delai_vent_alea = delai_vent_max*drand48();
    //if (delai_vent_max>=500){ //affichage de l'intensite seulement en frequence basse
      //LogPrintf("variation d'intensite du vent : lambda_E = %f   grdvc_min = %f   csp_time = %f\n", lambda_E, grdvc_min, csp_time);
      LogPrintf("variation d'intensite du vent : grdvc_min = %f   grdvc_max = %f   csp_time = %f\n", grdvc_min, grdvc_max, csp_time);
...
void simul_wind_data()
{
  ...
  if (start){
    ...
    if (time_unit == UNIT_T0){
      ...
      if (!param_is_set("Tau_min")) grdvc_min = wind_values[cpt_wind];
#ifdef TIME_SCALE
      //time scale
      simul_time_scale(-1);
#endif
    ...
    //LogPrintf("simul_wind_data: first direction = %f, time_threshold = %f\n", angle_dir,time_threshold);
  ...
  if (csp_time >= time_threshold){
    ...
    cpt_wind++;
    if (cpt_wind == nb_wind) cpt_wind = 0;
    //wind rotation
...
#ifdef LGCA
  if (angle) lgca_stabilize_flag = 1;
#endif
}
#endif //WIND_DATA
...
void simul_profile_lgca(int lgca_flag)
{
  ...
  if (lgca_flag == 0){
    elapsed(&t0);
    if (t1) other_elapsed_time += (t0-t1);
  }
  else if (lgca_flag == 1){
    elapsed(&t1);
...
void simul_lgca()
{
  ...
  if (lgca_stabilize_flag){
    if (lgca_reset) init_mvt();
    flow_stabilization(rot_mvt_nb_cycles);
  }

  /// lgca cycle
  if (rot_map) out_of_space_mvt(0);
  //LogPrintf("collision ...");
  do_collisions();
  //LogPrintf("propagation ...");
  do_propagations();
  //LogPrintf("end of lgca.\n");
  compute_vel(1);
...
int simul_csp()
{
  ...
  if (!wind_data_filename) simul_time_scale(-1);
#endif
...
#ifdef AVALANCHES
  if (ava_mode == AVA_SYNC) simul_ava_sync();

  if (ava_delay) simul_ava_dynamics();
#endif //AVALANCHES

#ifdef LGCA
  if (use_lgca){
    /// stabilization of lattice gas
    if (!param_is_set("Init_ncycl")) init_mvt_nb_cycles = 2*CLEO; //default value
    else if (init_mvt_nb_cycles<VSTEP_TIME) init_mvt_nb_cycles = VSTEP_TIME; //min value
    if (nom_fic_mvt){
    ...
    if (!param_is_set("Rot_ncycl")) rot_mvt_nb_cycles = lgca_reset && (!lgca_speedup) ? 2*CLEO : 3*VSTEP_TIME;
#ifdef CGV
    //init_cgv();
    if (opt_info) dump_cgv();
#endif
  }
#endif //LGCA
  ...
  while(!end_of_simul)
  {
    ...
    //for (i=0; i<100000/L; i++) calcule_alti(GR, ALTI_MODE_BAS); //pour ralentir ...
    ...
    if (use_lgca){
      if ((csp_time >= lgca_time_threshold) || lgca_stabilize_flag){
        ...
        if (mode_pat) wait_lgca_thread(); else simul_lgca();
  #else
        simul_lgca();
  #endif
  #ifdef ALTI
        ...
        while (lgca_time_threshold < csp_time) lgca_time_threshold += lgca_delay;
      }
    }
    else{
      simul_norm_lum();
  ...
  #endif
  #ifdef MODEL_LIFE
        LogPrintf("simul_csp: stopping simulation ...\n");
        end_of_simul = 1;
  #endif
      }
    }
...
#ifdef CGV
    //simul_wind_variability();
#endif
#ifdef WIND_DATA
    if (wind_data_filename) simul_wind_data();
#endif
...
#if defined(CELL_COLOR) && defined(MODEL_AVA)
   /*if (!(ii & 0x00007fff)){ //toutes les 65536 transitions
     init_color();
   }*/
#endif

#ifdef TRACE3D_CEL
    //if (!(ii & 0x000001fff)) trace3d_cel();  //toutes les 8192 transitions
    //if (!(ii & 0x00000000)) trace3d_cel();  //toutes les 256 transitions
    trace3d_cel();  //a chaque transition
#endif

#ifdef TRACE_FLUX
    extern float trace_flux_delay;
    if (trace_flux_delay) trace_dump(0);
#endif

#ifdef DUMP_SIGMA
    #define SIGMA_DELAY 1.0 //delay in t0 unit
    //if (!(ii & 0x000000fff)) dump_sigma_alti(); //toutes les 4096 transitions
    //if (!(ii & 0x0000000ff)) dump_sigma_alti(); //toutes les 256 transitions
    ...
    //if (!(ii & 0x00000ffff)) dump_autocorrel();
    ...
    if (mode_par && !(ii & 0xff)) {
      //if (synchro_terre()) {
        //LogPrintf("pause synchro\n");
        while (synchro_terre());
        //LogPrintf("synchro ok\n");
      //}
    }
#endif
...
void simul_dump()
{
  ...
  if (start){
    ...
    *str = 0;
#endif
    //time_threshold = csp_time;
    //set dump_delay = minimal non-zero delay
    if (dump_delay_png > 0){
      //time_threshold_dpng = csp_time;
    ...
    if (dump_delay_csp > 0){
      ...
      if (!dump_delay || (dump_delay_csp < dump_delay)) dump_delay = dump_delay_csp;
  ...
  if ((csp_time >= time_threshold) || end_of_simul){
    ...
    if (!t0 || (t1-t0 >= 0.1)){
      ...
      if (dump_delay_png && (time_threshold == time_threshold_dpng)){
        ...
        dump_image(name, "png");
        while (time_threshold_dpng <= csp_time) time_threshold_dpng += dump_delay_png;
      }
...
#endif
#if defined(LGCA) //&& !defined(ROTATIONS)
...
#endif
        while (time_threshold_dcsp <= csp_time) time_threshold_dcsp += dump_delay_csp;
      }
      ...
      if (time_threshold == time_threshold_dpng) time_threshold_dpng += dump_delay_png;
      if (time_threshold == time_threshold_dcsp) time_threshold_dcsp += dump_delay_csp;
      //LogPrintf("simul_dump: no dump)n");
    }

    //next time threshold
    while (time_threshold <= csp_time) time_threshold += dump_delay;
    if (dump_delay_png && (time_threshold_dpng < time_threshold)) time_threshold = time_threshold_dpng;
    if (dump_delay_csp && (time_threshold_dcsp < time_threshold)) time_threshold = time_threshold_dcsp;
    //LogPrintf("simul_dump: time_threshold = %f\n",time_threshold);
  }
}
