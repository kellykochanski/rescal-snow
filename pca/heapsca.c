...
cell_t* init_grid(int width, int height){
  ...
  for (int i = 0; i < width * height; i++){
    ...
    if ((i % width) == 0 || (i / width) == 0 || (i % width) == (width - 1) || (i / width) >= (width - 1))
      ...
      cell.val = arc4random_uniform(MAXGRIDVAL);
      while (cell.val == 0) cell.val= arc4random_uniform(MAXGRIDVAL);
    }
    cell.x = i / width;
    cell.y = i % width;
    *(grid_start + i) = cell;
...
cell_t* select_random_cell(cell_t* grid, int width, int height){
  cell_t* cell = (grid + (int) arc4random_uniform(width * height));
  while (cell->val == 0) cell = (grid + (int) arc4random_uniform(width * height));
  return cell;
}
...
void swap_cells(cell_t* curr_cell, int width /*, cell_t swaps[TOTALWIDTH][TOTALHEIGHT]*/){
...
int main(int argc, const char* argv[]){
  ...
  cell_t* grid_start = init_grid(width, height);
...
// BUGFIX - threads stepping on each other when swapping cells and/or selecting random #'s
  for (int i = 0; i < num_iter; i++){
    ...
    swap_cells(cell, width /*, swaps*/);
  }
 // display_grid(grid_start, width, height);
 // display_swaps(swaps);

  printf("Serial: Time to completion for %d iterations of a size %d x %d grid: %f \n", 
