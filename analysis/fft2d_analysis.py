...
def get_stats(timestep,threshold,fft_data,amps,d_freqs):
    ...
    return pd.DataFrame(stats,columns=['Time','Dominant Freq.','X','Y','Amplitude','Phase','Wavelength','Aspect_Ratio'])
...
#int -> the plot interval, e.g. 10 means every 10th element in the data list will be graphed and saved as png
...
def graph_all(int,dir,GIF_name,basename,data,graph_type,fig_size,x_label,y_label,z_label,title):
...
def main(directory="input_data/ALT_DATA1/", image_interval=100):

    MAIN_DATA_DIR = directory
    PNG_OUTPUT_DIR = "output_files/results_1/fft/png_output/"
    CSV_OUTPUT = "output_files/results_1/fft/fft_analysis.csv"
    GIF_OUTPUT = "output_files/results_1/fft/fft_results.gif"
    ...
    directories = [MAIN_DATA_DIR, PNG_OUTPUT_DIR, CSV_OUTPUT, GIF_OUTPUT]
    ...
    t0 = t.time()
    im_count = graph_all(SNAPSHOT_INTERVAL,PNG_OUTPUT_DIR,GIF_OUTPUT,BASE_FILE_NAME,all_amps,'surf',FIG_SIZE,XLABEL,YLABEL,ZLABEL,TITLE)
    t1 = t.time()
    t_plot = t1-t0
    t_total = t_read + t_fft2d + t_amps + t_freqs + t_stats + t_plot
    print("{} PNG's created in: {}s.\nGIF animation complete.\nAnalysis process complete!\nTotal time: {}s".format(im_count,t_plot,t_total))
